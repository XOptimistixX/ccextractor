...
struct encoder_cfg
{
	...
	int no_bom; 																				// Set to 1 when no BOM (Byte Order Mark) should be used for files.
																												//Note, this might make files unreadable in windows!
	char *first_input_file;
	int multiple_files;
	int no_font_color;
	int no_type_setting;
	int cc_to_stdout; 																	// If this is set to 1, the stdout will be flushed when data was
																													// written to the screen during a process_608 call.
