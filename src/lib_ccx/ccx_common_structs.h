...
struct cc_subtitle
{
	...
	*/
	void *data;
	unsigned int nb_data;							/** number of data */
	enum subtype type;								/**  type of subtitle */
	enum ccx_encoding_type  enc_type;	/** Encoding type of Text, must be ignored in case of
																					subtype as bitmap or cc_screen*/

	LLONG start_time;		/* set only when all the data is to be displayed at same time */
	LLONG end_time;

	int flags;					/* flags */
	int lang_index;			/* index of language table */
	int got_output;			/* flag to tell that decoder has given output */
