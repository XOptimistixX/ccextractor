...
enum cc_modes
{
	...
	MODE_TEXT = 		4,
	MODE_PAINTON = 	5,
	MODE_FAKE_ROLLUP_1 = 100 // Fake modes to emulate stuff
};
...
typedef struct eia608_screen // A CC buffer
{
	...
	LLONG start_time;		/** start time of this CC buffer */
	LLONG end_time;			/** end time of this CC buffer */
	enum cc_modes mode;
	int channel; 				// Currently selected channel
	int my_field; 			// Used for sanity checks

	char *xds_str;					 /** XDS string */
	size_t xds_len;					 /** XDS string length */
	int cur_xds_packet_class;/** XDS string class*/
} eia608_screen;
